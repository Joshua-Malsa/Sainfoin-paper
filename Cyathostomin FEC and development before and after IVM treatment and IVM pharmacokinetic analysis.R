...
#===== larval development, larval community structure, and efficacy of ivermectin treatment in horses ====

# == Effect on Cyathostomin FEC & development before and after IVM treatment and 
...
data_summary_Geo <- function(data, varname, groupnames){
  require(plyr)
  summary_func <- function(x, col){
    c(mean = geometric.mean(x[[col]], na.rm=TRUE),
      sd = sd(x[[col]], na.rm=TRUE))
  }
  data_sum<-ddply(data, groupnames, .fun=summary_func,
                  varname)
...
### === Graphs ===
#Boxplot
EPG <-ggplot(xpsainfoin, aes(x = Day ,y= EPG, fill=Group))+
...
### === Graphs ===
#Boxplot
Dev<-ggplot(xpsainfoin, aes(x = Day ,y= Dev, fill=Group))+
  geom_boxplot(alpha=1, outlier.shape = NA, size=0.9, width=0.8)+
...
####==== Merge of EPG and Dev figures ======
ggarrange(EPG, Dev, labels = c("A", "B"), common.legend = TRUE, legend = "bottom")
...
### === Graphs ===
Pk<- data_summary_Geo(PKivm, varname="Dosage",groupnames=c("Assay", "Time_hour"))
...
### === Statistical analysis of the effects of the diet on the IVM concentration ====
#Dosage data geometric average 
Pk
#       Assay Time_hour    Dosage        sd
# 1   Control         0 0.0000000 0.0000000
# 2   Control         1 0.6244794 0.6671698
# 3   Control         2 1.7549295 1.2621503
# 4   Control        24 6.6355850 1.7275146
# 5   Control        48 4.9988799 1.4483788
# 6   Control        72 3.3312978 1.2055686
# 7   Control        96 2.7378508 1.0606686
# 8  Sainfoin         0 0.0000000 0.0000000
# 9  Sainfoin         1 0.6026037 0.5669764
# 10 Sainfoin         2 1.4488465 1.3077190
# 11 Sainfoin        24 3.7514808 0.8499255
# 12 Sainfoin        48 2.6666804 1.0590462
# 13 Sainfoin        72 2.0368832 0.6939813
# 14 Sainfoin        96 1.5031276 0.7685600
...
stat.test <- PKivm[PKivm$Time_hour!="0",] %>%
  group_by(Time_hour) %>%
...
PKivmO<-PKivm[order(PKivm$Horses),]
df = PKivmO %>% data.table::dcast(Time_hour ~ Horses, value.var = 'Dosage')
df$Time_hour=NULL
dfl<-as.list(df)
...
DF=data.frame(Horses=PKivm[c(1:20),1],Group=PKivm[c(1:20),2],AUC="x")
...
##Dosage data geometric average 
AUC<- data_summary_Geo(DF, varname="AUC",groupnames="Group")
#      Group      AUC       sd
# 1  Control 408.5117 112.1007
# 2 Sainfoin 237.5212  76.0545
